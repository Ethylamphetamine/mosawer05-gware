...
extends WWidget {
    ...
    public boolean onKeyPressed(int key, int mods) {
        ...
        boolean bl = MinecraftClient.IS_SYSTEM_MAC ? mods == 8 : (control = mods == 2);
        if (control && key == 67) {
            if (this.cursor != this.selectionStart || this.cursor != this.selectionEnd) {
                MeteorClient.mc.keyboard.setClipboard(this.text.substring(this.selectionStart, this.selectionEnd));
            }
            return true;
        }
        if (control && key == 88) {
        ...
        } else if (mods == ((MinecraftClient.IS_SYSTEM_MAC ? 8 : 2) | 1) && key == 65) {
            this.resetSelection();
        } else {
            if (key == 257 || key == 335) {
                this.setFocused(false);
    ...
    public boolean onKeyRepeated(int key, int mods) {
        ...
        boolean control = MinecraftClient.IS_SYSTEM_MAC ? mods == 8 : mods == 2;
        boolean bl = shift = mods == 1;
        boolean controlShift = mods == ((SystemUtils.IS_OS_WINDOWS ? 4 : (MinecraftClient.IS_SYSTEM_MAC ? 8 : 2)) | 1);
        boolean bl2 = altShift = mods == ((SystemUtils.IS_OS_WINDOWS ? 2 : 4) | 1);
        if (control && key == 86) {
            this.clearSelection();
        ...
        if (key == 259) {
            if (this.cursor > 0 && this.cursor == this.selectionStart && this.cursor == this.selectionEnd) {
                String preText = this.text;
                int count = mods == (SystemUtils.IS_OS_WINDOWS ? 4 : (MinecraftClient.IS_SYSTEM_MAC ? 8 : 2)) ? this.cursor : (mods == (SystemUtils.IS_OS_WINDOWS ? 2 : 4) ? this.countToNextSpace(true) : 1);
        ...
        if (key == 261) {
            if (this.cursor == this.selectionStart && this.cursor == this.selectionEnd) {
                if (this.cursor < this.text.length()) {
                    String preText = this.text;
                    int count = mods == (SystemUtils.IS_OS_WINDOWS ? 4 : (MinecraftClient.IS_SYSTEM_MAC ? 8 : 2)) ? this.text.length() - this.cursor : (mods == (SystemUtils.IS_OS_WINDOWS ? 2 : 4) ? this.countToNextSpace(false) : 1);
        ...
        if (key == 263) {
            if (this.cursor > 0) {
                if (mods == (SystemUtils.IS_OS_WINDOWS ? 2 : 4)) {
                    ...
                    this.resetSelection();
                } else if (mods == (SystemUtils.IS_OS_WINDOWS ? 4 : (MinecraftClient.IS_SYSTEM_MAC ? 8 : 2))) {
                    this.cursor = 0;
                    this.resetSelection();
                } else if (altShift) {
    ...
    private void cursorChanged() {
        ...
        this.textStart = MathHelper.clamp((double)this.textStart, (double)0.0, (double)Math.max(this.textWidth() - this.maxTextWidth(), 0.0));
    ...
    private void createCompletions(int selected) {
        ...
        this.completionsW.calculateSize();
        this.completionsW.x = Math.min(Math.max(this.x - this.pad() * 2.0 + this.getTextWidth(this.cursor) - this.getOverflowWidthForRender(), this.x), this.x + this.width - this.completionsW.width);
        this.completionsW.y = this.y + this.height;
        this.completionsW.calculateWidgetPositions();
    }
    ...
    public void set(String text) {
        this.text = text;
        this.selectionStart = this.cursor = MathHelper.clamp((int)this.cursor, (int)0, (int)text.length());
        this.selectionEnd = this.cursor;
        this.calculateTextWidths();
        this.cursorChanged();
    }
