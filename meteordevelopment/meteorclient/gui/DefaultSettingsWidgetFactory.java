...
extends SettingsWidgetFactory {
    ...
    private void group(WVerticalList list, SettingGroup group, String filter, List<RemoveInfo> removeInfoList) {
        ...
        for (Setting<?> setting : group) {
            boolean visible;
            if (!StringUtils.containsIgnoreCase((CharSequence)setting.title, (CharSequence)filter)) continue;
    ...
    private void genericW(WTable table, GenericSetting<?> setting) {
        ...
        edit.action = () -> MeteorClient.mc.setScreen((Screen)((IScreenFactory)setting.get()).createScreen(this.theme));
    ...
    private void blockW(WTable table, BlockSetting setting) {
        WHorizontalList list = table.add(this.theme.horizontalList()).expandX().widget();
        WItem item = list.add(this.theme.item(((Block)setting.get()).asItem().getDefaultStack())).widget();
        ...
        select.action = () -> {
            ...
            screen.onClosed(() -> item.set(((Block)setting.get()).asItem().getDefaultStack()));
            MeteorClient.mc.setScreen((Screen)screen);
        };
        this.reset(table, setting, () -> item.set(((Block)setting.get()).asItem().getDefaultStack()));
    }
    ...
    private void itemW(WTable table, ItemSetting setting) {
        WHorizontalList list = table.add(this.theme.horizontalList()).expandX().widget();
        WItem item = list.add(this.theme.item(((Item)setting.get()).asItem().getDefaultStack())).widget();
        WButton select = list.add(this.theme.button("Select")).widget();
        select.action = () -> {
            ItemSettingScreen screen = new ItemSettingScreen(this.theme, setting);
            screen.onClosed(() -> item.set(((Item)setting.get()).getDefaultStack()));
            MeteorClient.mc.setScreen((Screen)screen);
        };
        this.reset(table, setting, () -> item.set(((Item)setting.get()).getDefaultStack()));
    }
    ...
    private void blockDataW(WTable table, BlockDataSetting<?> setting) {
        ...
        button.action = () -> MeteorClient.mc.setScreen((Screen)new BlockDataSettingScreen(this.theme, setting));
        this.reset(table, setting, null);
    }

    private void potionW(WTable table, PotionSetting setting) {
        WHorizontalList list = table.add(this.theme.horizontalList()).expandX().widget();
        WItemWithLabel item = list.add(this.theme.itemWithLabel(((MyPotion)((Object)setting.get())).potion, I18n.translate((String)((MyPotion)((Object)setting.get())).potion.getTranslationKey(), (Object[])new Object[0]))).widget();
        ...
        button.action = () -> {
            ...
            screen.onClosed(() -> item.set(((MyPotion)((Object)((Object)((Object)setting.get())))).potion));
            MeteorClient.mc.setScreen((Screen)screen);
        };
        this.reset(list, setting, () -> item.set(((MyPotion)((Object)((Object)setting.get()))).potion));
    }
