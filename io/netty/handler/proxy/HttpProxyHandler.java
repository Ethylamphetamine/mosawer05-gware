...
extends ProxyHandler {
    ...
    public HttpProxyHandler(SocketAddress proxyAddress, String username, String password, HttpHeaders headers, boolean ignoreDefaultPortsInConnectHostHeader) {
        super(proxyAddress);
        ByteBuf authzBase64;
        this.username = (String)ObjectUtil.checkNotNull((Object)username, (String)"username");
        this.password = (String)ObjectUtil.checkNotNull((Object)password, (String)"password");
        ByteBuf authz = Unpooled.copiedBuffer((CharSequence)(username + ':' + password), (Charset)CharsetUtil.UTF_8);
        try {
            authzBase64 = Base64.encode((ByteBuf)authz, (boolean)false);
        }
        finally {
            authz.release();
        }
